...
export async function adminAddPlayerToTournament(
  tournamentId: string,
  playerId: string,
  ...
  try {
    ...
    if (existing) return { ok: false, error: "Giocatore già iscritto" }

    const isFree = !tournament.priceCents || tournament.priceCents === 0
